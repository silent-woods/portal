...
namespace Nop.Plugin.ExchangeRate.EcbExchange
{
    public class EcbExchangeRateProvider : BasePlugin, IExchangeRateProvider
    {
        ...
        /// </returns>
        public async Task<IList<App.Core.Domain.Directory.ExchangeRate>> GetCurrencyLiveRatesAsync(string exchangeRateCurrencyCode)
        {
            if (exchangeRateCurrencyCode == null)
                throw new ArgumentNullException(nameof(exchangeRateCurrencyCode));

            //add euro with rate 1
            var ratesToEuro = new List<App.Core.Domain.Directory.ExchangeRate>
            {
                new App.Core.Domain.Directory.ExchangeRate
                {
                    CurrencyCode = "EUR",
                    Rate = 1,
            ...
            try
            {
                ...
                foreach (XmlNode currency in dailyRates.ChildNodes)
                {
                    ...
                    ratesToEuro.Add(new App.Core.Domain.Directory.ExchangeRate()
                    {
            ...
            return ratesToEuro.Select(rate => new App.Core.Domain.Directory.ExchangeRate
            {
