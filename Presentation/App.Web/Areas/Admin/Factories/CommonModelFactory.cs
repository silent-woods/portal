...
namespace App.Web.Areas.Admin.Factories
{
    ...
    public partial class CommonModelFactory : ICommonModelFactory
    {
        ...
        protected virtual async Task PreparePluginsWarningModelAsync(IList<SystemWarningModel> models)
        {
            ...
            var baseLibraries = new[] { "App.Core", "App.Data", "App.Services", "App.Web", "App.Web.Framework" };
        ...
        public virtual async Task<EmployeeOnLeaveListModel> PrepareEmployeeOnLeaveListModelAsync(EmployeeOnLeaveSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));

            // Get employees on leave
            var searchTermRecordLines = await _leaveManagementService.GetEmployeeOnLeaveList();

            // Convert tuple list to IList<EmployeeOnLeaveModel>
            var employeeOnLeaveList = searchTermRecordLines.Select(searchTerm => new EmployeeOnLeaveModel
            {
                Date = searchTerm.Date.ToString("dd-MMM-yyyy"),
                Emplyoee = searchTerm.EmployeeList
            }).ToList(); // Convert to List<T> (IList<T>)

            // Convert IList<EmployeeOnLeaveModel> to IPagedList<EmployeeOnLeaveModel>
            var pagedList = new PagedList<EmployeeOnLeaveModel>(
                employeeOnLeaveList,
                searchModel.Page - 1, 
            ...
            var model = await ModelExtensions.PrepareToGridAsync<EmployeeOnLeaveListModel, EmployeeOnLeaveModel, EmployeeOnLeaveModel>(
                new EmployeeOnLeaveListModel(),
                searchModel,
                pagedList,
                () => pagedList.ToAsyncEnumerable() // Ensure it's an async enumerable
            );
