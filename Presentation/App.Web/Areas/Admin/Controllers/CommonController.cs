...
namespace App.Web.Areas.Admin.Controllers
{
    public partial class CommonController : BaseAdminController
    {
        ...
        [HttpPost]
        public virtual async Task<IActionResult> EmployeeOnLeave(EmployeeOnLeaveSearchModel searchModel)
        {
            if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageLeaveManagement, PermissionAction.View))
                return await AccessDeniedDataTablesJson();

            //prepare model
            var model = await _commonModelFactory.PrepareEmployeeOnLeaveListModelAsync(searchModel);
